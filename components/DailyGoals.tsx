    ...
    <div className="glass rounded-2xl p-6">
      ...
      <div className="space-y-2 mb-4">
        {goals.map(g => (
          <div key={g.id} className="flex items-center gap-3">
            ...
            <button onClick={() => remove(g.id)} className="ml-auto text-gray-400 hover:text-red-400 text-xs">✕</button>
          </div>
        ))}
      ...
      <div className="flex gap-2">
        <input value={text} onChange={e => setText(e.target.value)}
          ...
          className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-white/30 placeholder:text-gray-500" />
        <button onClick={addGoal} className="px-4 py-2 bg-white hover:bg-gray-100 text-gray-900 rounded-lg text-sm transition">
          +
        </button>
      </div>
    </div>
